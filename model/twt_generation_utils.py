...
class TWTGenerationMixin(GenerationMixin):
    """
    ...
    def _expand_inputs_for_generation(
        ...
        **model_kwargs,
        ...
        # !!! Add decoder attention mask for batch generation
        if "decoder_attention_mask" in model_kwargs:
            decoder_attention_mask = model_kwargs["decoder_attention_mask"]
            model_kwargs["decoder_attention_mask"] = decoder_attention_mask.index_select(0, expanded_return_idx)

        # !!! Add decoder cross attention mask for TWT model
        if "decoder_cross_attention_mask" in model_kwargs:
            decoder_cross_attention_mask = model_kwargs["decoder_cross_attention_mask"]
            model_kwargs["decoder_cross_attention_mask"] = decoder_cross_attention_mask.index_select(0, expanded_return_idx)
        ...
        if is_encoder_decoder:
            ...
            # !!! Add input ids output for pointer and TWT model
            if "input_ids_output" in encoder_outputs:
                encoder_outputs["input_ids_output"] = encoder_outputs.input_ids_output.index_select(
                    0, expanded_return_idx.to(encoder_outputs.input_ids_output.device)
                )
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        # !!! Add prefix matching
        prefix_mask = None
        if 'decoder_masking_func' in model_kwargs and model_kwargs['decoder_masking_func'] is not None:
            masking_func = model_kwargs['decoder_masking_func']
            prefix_mask = masking_func()

        while cur_len < max_length:
            # prepare model inputs
            model_inputs = self.prepare_inputs_for_generation(input_ids, **model_kwargs)
            if prefix_mask is not None:
                model_inputs['decoder_prefix_mask'] = prefix_mask
            ...
            # !!! Add for probs instead of logits
            ...
            # !!! Changed for probes instead of logits
            next_tokens_scores = logits_processor(input_ids, next_token_scores)
            ...
            # !!! Update prefix mask
            if prefix_mask is not None:
                prefix_mask = masking_func([0], next_tokens[:, None][0].tolist())
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while cur_len < max_length:
            ...
            next_token_logits = outputs.logits[:, -1, :]
            # !!! Add for probs instead of logits
            next_token_scores = torch.log(next_token_logits)  # (batch_size * num_return_sequences, vocab_size)
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        while cur_len < max_length:
            ...
            # !!! Changed for probs instead of logits
            next_token_scores = torch.log(next_token_logits)  # (batch_size * num_beams, vocab_size)
