...
class CachedDataset():
    ...
    def __getitem__(self, index):
        ...
        # We have to make sure that the PAD token is ignored, BART uses 1 as pad_token_id
        input_item["labels"] = [-100 if token == 1 else token for token in input_item["labels"]]
...
class Seq2SeqTrainingArguments(TrainingArguments):
    ...
    adam_epsilon: float = field(default=1e-06, metadata={"help": "Epsilon for AdamW optimizer."})
...
class BartSeq2Seq(object):
    ...
    def _init_tokenizer(self):
        tokenizer = BartTokenizerFast.from_pretrained(f"facebook/bart-{self.args.model_size}", add_prefix_space=True)
        tokenizer.add_special_tokens({'additional_special_tokens': ADDITIONAL_SPECIAL_TOKENS})
        return tokenizer

    def _init_model(self):
        if not self.args.load_checkpoint:
            bart_config = TWTBartForConditionalGeneration.config_class.from_pretrained(
                f"facebook/bart-{self.args.model_size}",
            ...
            bart_config.type_vocab_size = 4
            bart_config.max_row_embeddings = 512
            bart_config.max_col_embeddings = 512
            bart_config.no_copy = self.args.no_copy
            bart_config.no_cross_attention_mask = self.args.no_cross_attention_mask
            bart_config.gen_loss_rate = self.args.gen_loss_rate
            bart_config.no_row_col_embeddings = self.args.no_row_col_embeddings

            # resize token embeddings
            bart_model = TWTBartForConditionalGeneration.from_pretrained(f"facebook/bart-{self.args.model_size}", config=bart_config)
            bart_model.resize_token_embeddings(len(self.tokenizer))
        else:
            # load model from checkpoint path
            bart_model = TWTBartForConditionalGeneration.from_pretrained(self.args.checkpoint_path)

        # sensible parameters for beam search
        bart_model.config.max_length = 50
        # bart_model.config.min_length = 10
        # bart_model.config.no_repeat_ngram_size = 3
        bart_model.config.early_stopping = True
        # bart_model.config.length_penalty = 2.0
        bart_model.config.num_beams = 4
...
def parse_args():
    ...
    parser.add_argument("--train_model_inputs_file",
                        type=str,
                        default="./data/cache/twt/totto_random_prefix_bart_base_train_model_inputs.pkl.gz",
                        help="Path of the train file")
    parser.add_argument("--val_model_inputs_file",
                        type=str,
                        default="./data/cache/twt/totto_random_prefix_bart_base_dev_model_inputs.pkl.gz",
                        help='Path of the validation file')
    parser.add_argument("--output_dir",
                        type=str,
                        default="./temp/output/totto_random_prefix_bart_base_twt_0.4_gen_loss",
